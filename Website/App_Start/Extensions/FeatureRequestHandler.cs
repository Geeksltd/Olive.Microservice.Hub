...
namespace System
{
    class FeatureRequestHandler
    {
        ...
        Feature FindFeature()
        {
            ...
            var withNullValues = Feature.All.Where(c => c == null || c.LoadUrl == null).ToArray();
            if (withNullValues.Any())
                throw new Exception(withNullValues.Select(c => c.Title).ToString(","));
    ...
    public static class FeatureRequestHandlerExtensions
    {
        public static IApplicationBuilder UseFeaturesViewPageMiddleware(this IApplicationBuilder @this)
        {
            ...
            {
                return new FeatureRequestHandler(context, next).Handle();
            });
        }
    }
}